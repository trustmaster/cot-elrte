	...
	elRTE.prototype.ui.prototype.buttons.code = function(rte, name) {
		...
		this.formatBlock = function(brush) {
			...
			function format(n, tag) {
				...
				if (/^(LI|DT|DD|TD|TH|CAPTION)$/.test(n.nodeName) && !self.rte.dom.isEmpty(n)) {
					self.rte.dom.wrapContents(replaceChilds(n), tag);
				} else if (/^(UL|OL|DL|TABLE)$/.test(n.nodeName)) {
					self.rte.dom.wrap(n, tag);
				} else if (!self.rte.dom.isEmpty(n) ) {
					$(replaceChilds(n)).replaceWith( $(self.rte.dom.create(tag)).html($(n).html()));
				}
...
elRTE.prototype.options.panels.myElements = ['horizontalrule', 'blockquote', 'div', 'stopfloat', 'nbsp', 'pagebreak', 'more'];
elRTE.prototype.options.toolbars.myToolbar = ['style', 'lists', 'links', 'elfinder', 'media', 'myElements', 'colors', 'undoredo', 'copypaste', 'alignment', 'format', 'indent', 'tables'];
var mySettings = {
	cssClass:		'el-rte',
	height:			450,
	toolbar:		'myToolbar',
	styleWithCSS:	false,
	cssfiles:		['plugins/elrte/css/elrte-inner.css'],
	fmAllow:        window.elFinder !== undefined,
	fmOpen:         function(callback) {
		elFmDialog = $('<div />').dialogelfinder({
			lang: mySettings.lang,
			url: 'index.php?r=elfinder&_ajax=1&x=' + $('input[name="x"]').val(),
			commandsOptions: {
				getfile: {
					oncomplete : 'destroy'
				}
